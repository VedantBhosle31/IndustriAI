...
def generate_strategies(user_input: str, portfolio: List[str]) -> List[Dict[str, Any]]:
    ...
    for strategy in strategies:
        strategy["recommendations"] = get_portfolio_recommendations(
            strategy["sectors"], portfolio
        )
...
def get_portfolio_recommendations(
    sectors: List[str], portfolio: List[str]
    ...
    try:
        ...
        if sector_metrics:
            ...
            buy_candidates = sorted(
                sector_metrics, key=lambda x: x.get("buy_score", 0), reverse=True
            )[:2]
            for stock in buy_candidates:
                signal = get_primary_signal(stock)
                recommendations["buy"].append(
                    {"ticker": stock["ticker"], "reason": signal}
                )
        ...
        if portfolio_metrics:
            ...
            sell_candidates = sorted(
                portfolio_metrics, key=lambda x: x.get("sell_score", 0), reverse=True
            )[:2]
        ...
        for section in strategy_sections:
            ...
            for line in lines:
                ...
                if line.startswith("Name:"):
                    ...
                    strategy["swot"]["strengths"] = line.replace(
                        "Strengths:", ""
                    ).strip()
                elif line.startswith("Weaknesses:"):
                    strategy["swot"]["weaknesses"] = line.replace(
                        "Weaknesses:", ""
                    ).strip()
                elif line.startswith("Opportunities:"):
                    strategy["swot"]["opportunities"] = line.replace(
                        "Opportunities:", ""
                    ).strip()
    ...
    for i, strategy in enumerate(strategies, 1):
        ...
        for action in ["buy", "sell"]:
            ...
            for rec in strategy["recommendations"][action]:
                print(
                    f"{'↑' if action == 'buy' else '↓'} {rec['ticker']}: {rec['reason']}"
                )
