...
def generate_trading_recommendations(
    metrics: Dict[str, float], stock_info: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    return {
        ...
        "metrics": {
            "value_21d": (
                f"+{metrics['value_change_21']:.1f}%"
                if metrics["value_change_21"] > 0
                else f"{metrics['value_change_21']:.1f}%"
            ),
            "risk_67d": (
                f"+{metrics['risk_change_67']:.1f}%"
                if metrics["risk_change_67"] > 0
                else f"{metrics['risk_change_67']:.1f}%"
            ),
            "esg_321d": (
                f"+{metrics['esg_change_321']:.1f}%"
                if metrics["esg_change_321"] > 0
                else f"{metrics['esg_change_321']:.1f}%"
            ),
        },
    }
...
def generate_stock_insights(
    ...
    return {"swot": swot_analysis, "analysis_date": datetime.now().strftime("%Y-%m-%d")}
    ...
    try:
        ...
        if stock_metrics:
            ...
            buy_candidates = sorted(
                stock_metrics, key=lambda x: x["value_21d"], reverse=True
            )
            # Sort by risk for sell recommendations (highest risk)
            sell_candidates = sorted(
                stock_metrics, key=lambda x: x["risk_67d"], reverse=True
            )
...
def get_stock_metrics(ticker: str) -> Dict[str, str]:
    ...
    try:
        ...
        return {"value_21d": "N/A", "risk_67d": "N/A", "esg_321d": "N/A"}
...
def main():
    ...
    for action_type in ["buy", "sell"]:
        ...
        for rec in signals[action_type]:
            print(
                f"{'↑' if action_type == 'buy' else '↓'} {rec['ticker']}: {rec['reason']}"
            )
