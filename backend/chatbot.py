...
def calculate_stock_metrics(hist: pd.DataFrame, ticker: str) -> Dict[str, float]:
    ...
    try:
        ...
        max_drawdown = (
            (hist["Close"] / hist["Close"].expanding(min_periods=1).max()) - 1
        ).min() * 100
        ...
        # Volume and momentum
        volume_trend = (
            hist["Volume"].iloc[-5:].mean() / hist["Volume"].iloc[-20:-5].mean() - 1
        ) * 100
        ...
        return {
            ...
            "risk_score": calculate_risk_score(
                volatility, downside_vol, rsi, esg_score
            ),
        }
...
def calculate_risk_score(
    volatility: float, downside_vol: float, rsi: float, esg_score: float
) -> float:
    ...
    esg_score_norm = (
        100 - esg_score
    ) / 100  # Convert ESG to risk (higher ESG = lower risk)

    return (
        vol_score * 0.3 + down_score * 0.3 + rsi_score * 0.2 + esg_score_norm * 0.2
    ) * 100
...
class PortfolioChat:
    ...
    def analyze_portfolio(self, portfolio: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if analysis["stock_metrics"]:
                analysis["portfolio_metrics"] = self._calculate_portfolio_metrics(
                    analysis["stock_metrics"]
                )
                analysis["sector_exposure"] = self._calculate_sector_exposure(portfolio)
                analysis["risk_profile"] = self._calculate_risk_profile(
                    analysis["stock_metrics"]
                )
                analysis["esg_metrics"] = self._calculate_esg_metrics(
                    analysis["stock_metrics"]
                )
                ...
                for report_name, analysis in self.report_analysis.items():
                    ...
                    if "metrics" in analysis:
                        ...
                        if metrics.get("margins"):
                            context += (
                                f"Latest Margin: {metrics['margins'][-1]['value']}%\n"
                            )
    ...
    def _calculate_portfolio_metrics(
        self, stock_metrics: Dict[str, Dict]
    ) -> Dict[str, float]:
        ...
        try:
            ...
            if n_stocks > 0:
                metrics["total_return"] = (
                    sum(m["value_change_21d"] for m in stock_metrics.values())
                    / n_stocks
                )
                metrics["portfolio_volatility"] = (
                    sum(m["volatility"] for m in stock_metrics.values()) / n_stocks
                )
                metrics["avg_esg_score"] = (
                    sum(m["esg_score"] for m in stock_metrics.values()) / n_stocks
                )
                metrics["avg_risk_score"] = (
                    sum(m["risk_score"] for m in stock_metrics.values()) / n_stocks
                )
                metrics["max_drawdown"] = min(
                    m["max_drawdown"] for m in stock_metrics.values()
                )
        except Exception as e:
    ...
    def _calculate_sector_exposure(self, portfolio: List[str]) -> Dict[str, float]:
        ...
        try:
            for ticker in portfolio:
                ...
                if sector:
                    exposure[sector] = (
                        exposure.get(sector, 0) + (1 / len(portfolio)) * 100
                    )
        except Exception as e:
    ...
    def _calculate_risk_profile(self, stock_metrics: Dict[str, Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            profile["volatility_risk"] = sum(m["volatility"] for m in metrics) / len(
                metrics
            )
            profile["drawdown_risk"] = min(m["max_drawdown"] for m in metrics)
            profile["esg_risk"] = 100 - sum(m["esg_score"] for m in metrics) / len(
                metrics
            )
            profile["climate_risk"] = sum(m["climate_risk"] for m in metrics) / len(
                metrics
            )
            profile["governance_risk"] = sum(
                m["governance_risk"] for m in metrics
            ) / len(metrics)
        except Exception as e:
    ...
    def _calculate_esg_metrics(
        ...
        try:
            ...
            metrics["avg_climate_risk"] = sum(m["climate_risk"] for m in values) / len(
                values
            )
            metrics["avg_governance_risk"] = sum(
                m["governance_risk"] for m in values
            ) / len(values)
        except Exception as e:
    ...
    def _create_chat_context(self) -> str:
        ...
        if self.current_analysis["portfolio_metrics"]:
            ...
            context.append(
                f"- Total Return (21d): {metrics.get('total_return', 0):0.1f}%"
            )
            context.append(
                f"- Portfolio Volatility: {metrics.get('portfolio_volatility', 0):0.1f}%"
            )
            context.append(
                f"- Average ESG Score: {metrics.get('avg_esg_score', 0):0.1f}"
            )
        ...
        if self.current_analysis["risk_profile"]:
            ...
            context.append("\nRisk Profile:")
            context.append(
                f"- Volatility Risk: {profile.get('volatility_risk', 0):0.1f}%"
            )
            context.append(
                f"- Maximum Drawdown: {profile.get('drawdown_risk', 0):0.1f}%"
            )
    ...
    def _generate_completion(self, prompt: str) -> str:
        ...
        try:
            ...
            api_key = "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"
            ...
            return (
                "Please set up your GROQ_API_KEY environment variable for AI analysis."
            )
                ...
                for pattern in pattern_list:
                    ...
                    for match in matches:
                        if category == "revenue":
                            ...
                            margin_type = (
                                match.group(1) if len(match.groups()) > 1 else "margin"
                            )
                            margin_value = float(match.group(-1))
                            metrics["margins"].append(
                                {"type": margin_type, "value": margin_value}
                            )

                        elif category == "growth":
                            growth_type = (
                                match.group(1) if len(match.groups()) > 1 else "growth"
                            )
                            growth_value = float(match.group(-1))
                            metrics["growth_rates"].append(
                                {"type": growth_type, "value": growth_value}
                            )
                            ...
                            metrics["ratios"].append(
                                {"type": ratio_type, "value": ratio_value}
                            )
                            ...
                            metrics["currency_values"].append(
...
def main():
    ...
    response = chat.process_message(
        "Analyze my portfolio's risk profile and suggest improvements.", portfolio
    )
    print("\nAI Response:")
    print(response)
    ...
    # Analyze report
    chat.analyze_report(
        "C:/Users/Asus/Downloads/Stock-Risk-Analysis-master/Stock-Risk-Analysis-master/sample_quarterly_report.pdf"
    )

    # Ask questions about both portfolio and report
    response = chat.process_message(
        "What are the key risks based on both the portfolio and recent reports?"
    )
    print(response)
